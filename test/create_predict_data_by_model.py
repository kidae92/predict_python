...
def get_influx(db, host='dbserver', port=8086, user='user', passwd='pwd'):
...
def startPredict(client, sc, model, json_body):
    ...
    for i in range(3):
        ...
        json_34 = input_json(json_body, msm_curr,
                             float(temp), float(hum), time)
        client.write_points(json_34)

    current_time = datetime.datetime.utcnow()
    # print(current_time)
    future_time = current_time + \
        datetime.timedelta(seconds=30) + datetime.timedelta(hours=9)
    print(future_time)
